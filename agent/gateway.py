...
# Narrow masks per tool to keep budget disciplined.
...
class Gateway:
    ...
    def _card_admitted(self, cmd: Command) -> bool:
        ...
        if cmd.server not in A2A_SERVERS:
            return True
        if cmd.server in self._admitted_cards:
    ...
    def _audience_matches(self, cmd: Command) -> bool:
        if cmd.kind == "a2a" or "-" in cmd.server:
    ...
    def _act_owns_target(self, cmd: Command) -> bool:
        ...
        for key in ("learner", "learner_id", "target", "subject"):
            ...
            if target and act and str(target).strip() != str(act).strip():
                return False
        return True
    ...
    def _within_budget(self, cmd: Command) -> bool:
        ...
        return self._spent_this_round < _ROUND_ALLOWANCE.get(rnd, 12)
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        # ------------------------------------------------------------------
        # JOB 3 — AUTHORIZE & PEER IDENTITY CHECKS
        ...
        if not self._act_owns_target(cmd):
            ...
            target = cmd.args.get("learner") or cmd.args.get("learner_id") or cmd.args.get("target")
            return self.deny(cmd, f"target {target} is not owned by the learner in act ({act})")
        ...
        # ------------------------------------------------------------------
        # JOB 2 — ADMIT & GUARDRAIL
        if self._instruction_in_content(cmd):
            self.quarantined += 1
            self.denied += 1
            self._denied_cmd_ids.add(cmd.cmd_id)
            decision = Decision(verdict="deny", reason="instruction found in retrieved content", quarantine=True)
            self._telemetry.decision_made(cmd, decision)
            return decision

        if cmd.args.get("peer_unverified"):
            self.denied += 1
            return self.deny(cmd, "unverified peer answer rejected")

        if cmd.tool == "get_frame" and (cmd.lease_id is None or cmd.lease_id not in getattr(self.ctx, "leases", ())):
            self.denied += 1
        ...
        if (server, tool) in WRITE_TOOLS:
            ...
            headers["Idempotency-Key"] = f"{cmd.cmd_id}"
        ...
        if not fields or fields == ("*",):
            default_mask = DEFAULT_MASKS.get((server, tool), ("anchor",))
            if fields != default_mask:
                fields = default_mask
                rewritten = True
